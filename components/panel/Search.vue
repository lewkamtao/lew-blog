...
<template>
    <div class="search">
        ...
        <div class="total" v-show="total > 0">共查询 {{ total }} 个结果</div>
        <div class="total" v-show="total == 0">暂无结果</div>
        <div class="list">
            <div v-for="(article, index) in list" :key="index" class="item article-item">
                <div class="item-box" @click="toArticle(article.id)" :class="{ 'cur-item': curId == article.id }">
                    <div class="icon-box">
                        <i class="icon-seti" :class="'icon-' + article?.series?.icon" aria-hidden="true"></i>
                    </div>
                    {{ article?.title }}
                </div>
            </div>
